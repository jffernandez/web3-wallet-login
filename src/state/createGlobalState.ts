...
export const createGlobalState = <State extends object>(initialState: State) => {
  ...
  const setGlobalState = <StateKey extends StateKeys>(stateKey: StateKey, update: SetStateAction<State[StateKey]>) => {
  ...
  const useGlobalState = <StateKey extends StateKeys>(stateKey: StateKey) => {
    ...
    const updater = useCallback((u: SetStateAction<State[StateKey]>) => setGlobalState(stateKey, u), [stateKey])
